...
void fillWay(Map* map, Way * way, SDL_Renderer* renderer){
  ...
  if(way->tag!=NULL){
    if(strcmp(way->tag->tagKey,"highway")==0){
      ...
      polygonRGBA(renderer,coord_x,coord_y,nbre,139,71,137,240);

      filledPolygonRGBA(renderer,coord_x,coord_y,nbre,way->tag->c->red,way->tag->c->green,way->tag->c->blue,240);
    }
  }
  else{
    filledPolygonRGBA(renderer,coord_x,coord_y,nbre,0,0,255,200);
  }
}
